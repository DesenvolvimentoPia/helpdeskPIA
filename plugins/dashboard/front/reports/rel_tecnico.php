	...
	<table id='tec' class='display' style='font-size: 13px; font-weight:bold;' cellpadding = 2px >
		<thead>
			<tr>
				...
				<th style='text-align:center; cursor:pointer;'> ". __('Resolution') ."</th>
...
while($row = $DB->fetch_assoc($result_cham)){
	...
	if($satisfacao != '' || $satisfacao > 0) {
		...
		<td style='vertical-align:middle; text-align:left;'> ". conv_data_hora($row['date']) ." </td>
		<td style='vertical-align:middle; text-align:left;'> ". conv_data_hora($row['closedate']) ." </td>
		<td style='vertical-align:middle; text-align:left;'> ". time_ext($row['time']) ."</td>
        ...
        "sDom": 'T<"clear">lfrtip',
         ...
         "aButtons": [
             ...
             },
             {
                 ...
                 "sPdfMessage": "<?php echo __('Technician', 'dashboard'); ?> : <?php echo $tech . '  - '; ?> <?php echo  __('Tickets','dashboard'); ?> : <?php echo $conta_cons . '  - ' ; ?>  <?php echo  __('Period','dashboard'); ?> : <?php echo conv_data($data_ini2); ?> - <?php echo conv_data($data_fin2); ?>"
                  } ]
             }
         ]
        }
